...
- **`routes/rag_routes.py`** — HTTP layer. Endpoints (all under prefix `/api/v1/`): `GET /status` (open), `POST /upload` (multipart: `channel_id` form field + `file`), `POST /chat` (JSON `{channel_id, message, filename?}`). `/upload` and `/chat` carry `Depends(require_api_key)` + a slowapi rate-limit decorator and take a `request: Request` param (slowapi requires it). Standardized JSON envelope (`success`/`message`/`data`/`error`) via `create_error_response`. On upload it registers the doc in the Redis channel manifest.
- **`controller/rag_controller.py`** — orchestration. `create_document_embeddings(channel_id, file_path)` chunks text and **dual-writes**: upserts into the channel's Chroma collection AND appends to the channel BM25 index. `chat_with_document` is the explicit hybrid flow: validate → `load_embeddings(channel_id)` (404 if none) → load Redis history → `contextualize_question` (LLM rewrite, skipped — saving one Groq round-trip — until there's at least one full prior exchange in history) → `HybridRetriever.retrieve` → if no docs, return a grounded fallback **without** calling the LLM → else `answer` (LLM) → append+save history → envelope.
...
- **`retrieval/`** — the retrieval engine. `chunking.py` (`chunk_text` → `Document`s tagged `{channel_id, source, doc_id, chunk_id}`, stable `compute_doc_id`); `bm25_index.py` (per-channel BM25 corpus persisted at `data/database/<channel_id>/bm25.pkl`; `add_documents` / `search`, in-process index cache keyed by channel, invalidated on `add_documents`); `fusion.py` (`reciprocal_rank_fusion`, keyed on `chunk_id`); `reranker.py` (`CrossEncoderReranker`, despite the name now calls the **Cohere Rerank API** (`settings.RERANKER_MODEL`, `settings.COHERE_API_KEY`) instead of running a local cross-encoder — swapped for latency, local CPU-only inference was ~2.5-4.7s per request vs. ~0.6-1s over the network); `hybrid_retriever.py` (`HybridRetriever(channel_id, vectorstore).retrieve(query, filename=None)` → dense+BM25 → RRF → rerank, dense errors degrade to sparse-only).
...
- **`utilities/rag_utilities.py`** — manages the embedding model + **two** class-cached Groq LLM instances (`self.llm` = `GROQ_MODEL`, full quality, used for `answer`; `self.fast_llm` = `GROQ_FAST_MODEL`, used only for the cheap `contextualize_question` rewrite), loads/caches per-channel Chroma stores (`load_embeddings(channel_id)`), and holds the chat helpers `contextualize_question(message, history)` and `answer(user_input, context, history, filename)` plus the prompt builders (`_contextualize_prompt`, `create_qa_prompt`).
...
- **`config/settings.py`** — pydantic-settings `Settings` singleton (`settings`), all paths relative to `BASE_DIR`. Knobs: `CHUNK_SIZE`, `CHUNK_OVERLAP`, `CHROMA_COLLECTION_NAME`, `CHANNEL_TTL_SECONDS` (P1); `DENSE_TOP_K`, `BM25_TOP_K`, `RRF_K`, `RERANK_TOP_N`, `RERANKER_MODEL` (Cohere model id, e.g. `rerank-english-v3.0`), `COHERE_API_KEY` (P2); `API_KEYS`, `RATE_LIMIT_CHAT`, `RATE_LIMIT_UPLOAD`, `ENABLE_QUERY_CACHE`, `QUERY_CACHE_TTL`, `METRICS_ENABLED` + `api_keys_list()` (P3); `GROQ_FAST_MODEL` (small/fast Groq model for query rewriting, separate from the full-quality `GROQ_MODEL`). **`config/logger.py`** — preconfigured loguru `logger`; import this, not loguru directly.
...
- **Caching is process-global and stateful.** `RAGUtilities` caches the LLM, fast LLM, and embedding model at the class level; `VECTOR_STORE_CACHE`, `CrossEncoderReranker._client` (Cohere client), and the BM25 index cache are process-global. Not shared across workers, not restart-surviving.
- **Chat latency.** Two Groq LLM round-trips (`contextualize_question` + `answer`) and one Cohere Rerank call are the dominant per-request cost. Warm-path total is ~1-3s depending on whether the rewrite step runs; the reranker was moved off local CPU inference (was ~2.5-4.7s alone on this CPU-only-torch environment) to Cohere's hosted API specifically to cut this down.
...
- **`transformers` must be >= 4.51.0.** Docling's default layout model (`docling-layout-heron`) declares `model_type: rt_detr_v2` in its config, which older `transformers` releases (this project was pinned to 4.48.3) don't recognize — parsing fails with `Transformers does not recognize this architecture`. Confirmed 4.51.0 has `rt_detr_v2` in its model registry; bumped the pin accordingly.
...
**Do not use Starlette's `TestClient` for route tests in this repo** — it hangs indefinitely here. Confirmed via `faulthandler` stack dump: its blocking-portal worker thread runs a Windows `ProactorEventLoop` (IOCP-based) that sits idle in `select()` forever — the submitted request coroutine never gets scheduled onto it. Ruled out Sentry as a cause (disabling/removing it entirely made no difference, and Sentry has since been removed from the project anyway) — this looks like a Windows-specific `asyncio`/`anyio` blocking-portal bug, not an application issue. Use `httpx.AsyncClient` with `httpx.ASGITransport(app=app)` directly instead (async test, `pytest.mark.anyio`) — see `tests/test_crawl_route.py` for the pattern.
